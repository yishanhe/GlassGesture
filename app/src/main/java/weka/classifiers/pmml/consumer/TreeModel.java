...
public class TreeModel extends PMMLClassifier implements Drawable {
  ...
  static class SimplePredicate extends Predicate {
    ...
    enum Operator {
      ...
      abstract Eval evaluate(double[] input, double value, int fieldIndex);
  ...
  static class CompoundPredicate extends Predicate {
    ...
    enum BooleanOperator {
      OR("or") {
        Eval evaluate(ArrayList<Predicate> constituents, double[] input) {
      ...
      abstract Eval evaluate(ArrayList<Predicate> constituents, double[] input);
...
//      Instances totalStructure = miningSchema.getFieldsAsInstances();
  ...
  static class SimpleSetPredicate extends Predicate {
    ...
    enum BooleanOperator {
        IS_IN("isIn") {
          Eval evaluate(double[] input, int fieldIndex,
        ...
        IS_NOT_IN("isNotIn") {
          Eval evaluate(double[] input, int fieldIndex,
              Array set, Attribute nominalLookup) {
            Eval result = IS_IN.evaluate(input, fieldIndex, set, nominalLookup);
